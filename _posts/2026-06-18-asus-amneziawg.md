 ...
 * **Несколько гео-политик через вкладки** - у каждой свой набор списков, каждое устройство выбирает свою; есть и режим «всё в VPN, кроме перечисленного».
 * **Не утекает и не роняет сеть** - kill-switch, самовосстановление маршрутов и DNS, безопасное «Применить» без обрыва интернета.
 * **Уживается с zapret, Xray, b4 и др.** - распознаёт соседние обходы и не конфликтует с ними по DNS.
 * **Ставится и обновляется даже при заблокированном GitHub** - через зеркала, через VPN или вручную из браузера.
 * **Анализатор трафика** - живой лог запросов устройства с пометкой «напрямую / через VPN».
 * **Удобнее в работе** - глобальный значок статуса в шапке, интерфейс RU/EN и тёмная тема.
 * **Оптимизация awg** - на старых роутерах работает без падений [Пул реквест в amneziawg-go](https://github.com/amnezia-vpn/amneziawg-go/pull/152)
