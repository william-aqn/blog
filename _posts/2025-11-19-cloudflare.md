...
## Пробема 1
Они не меняли выборку и условия, они поменяли настройки прав. И запрос, который раньше возвращал данные только для одной БД, стал возвращать от нескольких БД. То есть, раньше все нормально работало и без условия, а после изменения настроек прав - тот же самый запрос, который раньше нормально работал, стал выдавать больше данных, что привело к проблемам.
